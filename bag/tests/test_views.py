...
class BagFlowTests(TestCase):
    def setUp(self):
        self.product = Product.objects.create(name="Test Cake", price=Decimal("9.99"))
    ...
    def test_add_adjust_remove(self):
        self.client.post(reverse('add_to_bag', args=[self.product.id]), {'quantity': 2, 'redirect_url': reverse('view_bag')})
        self.assertEqual(self.client.session['bag'][str(self.product.id)], 2)
        self.client.post(reverse('adjust_bag', args=[self.product.id]), {'quantity': 1})
