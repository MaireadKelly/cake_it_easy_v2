...
def _ids_for_category_and_children(slug):
    """Return [parent.id] + children ids for a Category slug. If missing, return ([], None)."""
    try:
...
def product_cakes(request):
    ...
    qs = Product.objects.filter(category_id__in=ids) if ids else Product.objects.none()
...
def product_offers(request):
    ...
    qs, q, sort, direction = _apply_search_sort(Product.objects.filter(is_offer=True), request)
    context = {
        'products': qs,
