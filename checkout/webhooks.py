...
def stripe_webhook(request):
    ...
    if not secret:
        ...
        logger.warning("Stripe webhook received but no STRIPE_WEBHOOK_SECRET set")
    ...
    logger.warning("Stripe webhook received: %s", event_type)

    if event_type == "payment_intent.succeeded":
        intent = event["data"]["object"]
        pid = intent.get("id")

        logger.warning("payment_intent.succeeded pid=%s", pid)

        if not pid:
            logger.warning("payment_intent.succeeded missing pid")
            return HttpResponse(status=200)

        try:
            order = Order.objects.get(stripe_pid=pid)
            logger.warning("Order found for pid=%s (paid=%s)", pid, order.paid)

            if not order.paid:
                order.paid = True
                order.save(update_fields=["paid"])
                logger.warning("Order marked paid for pid=%s", pid)
            ...
            logger.warning("No matching order found for pid=%s", pid)

        except Exception:
            logger.exception("Unexpected error handling payment_intent.succeeded")
